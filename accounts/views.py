        ...
        if form.is_valid():
            ...
            if profile_id is not None:
                ...
                coupon = CouponCode.objects.create(
                    user=user, coupon_name=coupon_name, off_price=offer_price, valid_from=today, valid_upto=third_day, valid=True)
                coupon2 = CouponCode.objects.create(
                    user=recom_user, coupon_name=coupon_name, off_price=offer_price, valid_from=today, valid_upto=third_day, valid=True)
                coupon.save()
                coupon2.save()
...
def user_profile(request, id):
    if request.user.is_authenticated:
        ...
        try:
            is_saved = Address.objects.filter(
                user=request.user, user_id=id).exists()
        ...
        if request.method == 'POST':
            ...
            if Profile.objects.filter(user=request.user).exists():
                ...
                img_data1 = ContentFile(base64.b64decode(
                    img1), name=user.first_name + '1.' + ext)
                ...
                prof_image = Profile(
                    user=request.user, profile_image=img_data1)
                prof_image.save()
    ...
    if request.user.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                address = Address(user=request.user, first_name=firstname, last_name=lastname,
...
def admin_home(request):
    if session_check(request):
        ...
        order_cancel = Order.objects.filter(
            is_ordered=False, status='cancelled',)
        order_pend = Order.objects.filter(is_ordered=True, status='pending',)
        order_place = Order.objects.filter(
            is_ordered=True, status='Order placed',)
        ...
        total_profit = 0
        sales_today = Order.objects.filter(is_ordered=True, created_at__lte=datetime.datetime.today(
        ), created_at__gt=datetime.datetime.today()-datetime.timedelta(days=7))

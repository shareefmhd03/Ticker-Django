...
def product_view(request,category_slug,product_slug):
    ...
    try:
        product_view = Product.objects.get(category__slug = category_slug, slug = product_slug)
        category_view = Category.objects.get(slug =category_slug)
        
        in_cart = CartItem.objects.filter(cart__cart_id= _cart_id(request), product= product_view).exists()
        try:
            product_offer_exists = ProductOffer.objects.filter(product = product_view.id).exists()  
            category_offer_exists = CategoryOffer.objects.filter(category = category_view.id).exists()     
    ...
    try:
        ...
        print('in____try')
        order_product  = OrederProduct.objects.filter(user__id= user.id, product_id = product_view.id).exists()
        print(order_product)
    ...
    reviews = ReviewRating.objects.filter(product_id = product_view.id, status = True)
    ...
    if session_check(request):
        ...
        if request.method    == 'POST':     
            ...
            if  form.is_valid():                  
                ...
                ext = format.split('/')[-1]
                img_data1 = ContentFile(base64.b64decode(img1), name= p_name + '1.' + ext)
            
                format, img2 = image2.split(';base64,')
                ext = format.split('/')[-1]
                img_data2 = ContentFile(base64.b64decode(img2), name=p_name + '2.' + ext)

                format, img3 = image3.split(';base64,')
                ext = format.split('/')[-1]
                img_data3 = ContentFile(base64.b64decode(img3), name=p_name + '3.' + ext)

                format, img4 = image4.split(';base64,')
                ext = format.split('/')[-1]
                img_data4 = ContentFile(base64.b64decode(img4), name=p_name + '4.' + ext)
...
def order_management(request):
    if session_check(request):
        order =Order.objects.exclude(payment__payment_method = None).order_by('-created_at')
...
def weekly_report(request):
    if request.method == 'POST':
        ...
        items = OrederProduct.objects.filter(created_at__gt = date_from, created_at__lt = date_upto).order_by('-created_at')
    else:
        items = OrederProduct.objects.filter(created_at__gt=datetime.datetime.today()-datetime.timedelta(days=7)).order_by('-created_at')
    context ={
        'order':items,
    }
...
def monthly_report(request):
    if request.method == 'POST':
        ...
        items = OrederProduct.objects.filter(created_at__gt=datetime.datetime.today()-datetime.timedelta(days=30)).order_by('-created_at')
    context ={
        'order':items,
    }
...
def yearly_report(request):
    if request.method == 'POST':
        ...
        items = OrederProduct.objects.filter(created_at__gt=datetime.datetime.today()-datetime.timedelta(days=365)).order_by('-created_at')
...
def add_product_offer(request, product_id):
    ...
    if request.method  == 'POST': 
        ...
        prod_offer = ProductOffer(offer_name=offer_name,off_percentage=off_percentage, product_id = product_id, valid_from =valid_from,valid_upto=valid_upto ,expired = False)
...
def add_category_offer(request):
    ...
    if request.method  == 'POST': 
        ...
        cat_offer = CategoryOffer(offer_name=offer_name,off_percentage=off_percentage, category_id = cat, valid_from =valid_from,valid_upto=valid_upto, expired = False )     
        cat_offer.save()
